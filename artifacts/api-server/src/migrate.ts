...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname = 'assets_serial_number_unique'
  ) THEN
...
async function countRows(client: Awaited<ReturnType<typeof pool.connect>>, table: string): Promise<number> {
...
export async function runMigrations(): Promise<void> {
  ...
  try {
    ...
    if ((await countRows(client, "employees")) === 0) {
      await client.query(
        `INSERT INTO employees (name, email, department, role, phone, status) VALUES ${EMPLOYEES.join(",")}`
      );
      logger.info(`Employees seeded ✓`);
    }

    if ((await countRows(client, "assets")) === 0) {
      await client.query(
        `INSERT INTO assets (name, category, brand, model, serial_number, status, condition, location, purchase_date, warranty_expiry, purchase_price, assigned_to_id) VALUES ${ASSETS.join(",")}`
      );
      logger.info(`Assets seeded ✓`);
    }

    if ((await countRows(client, "tickets")) === 0) {
      await client.query(
        `INSERT INTO tickets (title, description, status, priority, category, assigned_to_id, requester_id) VALUES ${TICKETS.join(",")}`
      );
