...
export async function runMigrations(): Promise<void> {
  ...
  try {
    ...
    // Seed each table independently so a partial seed can be resumed
    const { rows: empRows } = await client.query("SELECT count(*)::int AS n FROM employees");
    if (empRows[0].n === 0) {
      await client.query(SEED_EMPLOYEES_SQL);
      logger.info("Employees seeded ✓");
    }

    const { rows: assetRows } = await client.query("SELECT count(*)::int AS n FROM assets");
    if (assetRows[0].n === 0) {
      await client.query(SEED_ASSETS_SQL);
      logger.info("Assets seeded ✓");
    }

    const { rows: ticketRows } = await client.query("SELECT count(*)::int AS n FROM tickets");
    if (ticketRows[0].n === 0) {
